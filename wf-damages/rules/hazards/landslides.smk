...
rule rasterise_landslide:
    """
    snakemake --cores 4 ../results/hazards/input/landslide_2050_ssp245_rp00100.tif
    """
    input:
    ...
    run:
        ...
        gdf = gdf.to_crs(params.crs)
        gdf['geometry'] = gdf.geometry.buffer(500)
        gdf = gdf.sort_values('hazard_score', ascending=True)
        gdf = gdf.to_crs(4326)
...
rule all_landslide_historical:
    """
    snakemake --cores 4 all_landslide_historical
    """
    input:
        tiffs = expand(
            "../results/hazards/input/{SUBCATEGORY}_{EPOCH}_{SCENARIO}_rp{RP}.tif",
