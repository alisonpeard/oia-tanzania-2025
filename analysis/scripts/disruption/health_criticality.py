...
Using max speed of 50 kmph because all data had speeds below this.
Re-route traffic for every flood hazard. Treats hazards as maps.
Urban Areas: The speed limit is generally 50 km/h (31 mph) in cities and towns, though lower limits may apply in certain areas, such as near schools or hospitals.
Rural Roads: On non-urban roads, the speed limit is typically 80 km/h (50 mph).
Highways: The speed limit on highways is usually 100 km/h (62 mph).

https://www.lawgratis.com/blog-detail/motor-vehicle-laws-tanzania#:~:text=Urban%20Areas%3A%20The%20speed%20limit,%2Fh%20(62%20mph).
"""
# %%
import os
import time
...
pops_col = "population"
max_cost = [30.0, np.inf][0]  # 2 hours for Bellweather, 30 mins for primary care
...
if __name__ == "__main__":
    ...
    weights = roads["travel_time"].to_numpy(dtype=np.float64)
    ...
    pops_df = pops_df[["id", "health", pops_col]].rename(columns={pops_col: "pop"}).copy()
    pops_df["idx"] = pops_df["id"].map(id_to_index)
    pops_df = pops_df.dropna(subset=["idx"]).copy()
    pops_df["idx"] = pops_df["idx"].astype(np.int64)
    pops_df = pops_df.set_index("idx")[["health","pop"]].sort_index()
    pops = np.zeros(n_vertices, dtype=np.float64)
    for idx, row in pops_df.iterrows():
        pops[idx] = row["pop"]

    # split origin/destination nodes
    origin_nodes = pops_df[pops_df["health"] == 0].index.to_numpy(dtype=np.int32)
    dest_nodes = pops_df[pops_df["health"] == 1].index.to_numpy(dtype=np.int32)
    ...
    out_df.to_csv("~/Desktop/tza_health_roads_edge_criticality.csv", index=False)
    # %% next: local detours
    if False:
