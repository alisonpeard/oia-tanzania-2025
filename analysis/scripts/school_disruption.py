...
if __name__ == "__main__":
    ...
    for hazard in (pbar := tqdm(relevant_hazards)):
        ...
        for file in hazfiles:
            ...
            df["subregion"] = os.path.basename(os.path.dirname(file))
            haz_dfs.append(df)

        haz_df = pd.concat(haz_dfs).sort_index()
        damage_cols = [col for col in haz_df.columns if col.startswith("damage-")]
        ...
        for i, col in enumerate(damage_cols):
            print(f"Processing columnn {i} / {len(damage_cols)}: {col}")
            damaged = haz_df[haz_df[col] > 0].copy()
        ...
        summary = summary.sort_values(by=["hazard", "epoch", "scenario", "rp", "stat"])
