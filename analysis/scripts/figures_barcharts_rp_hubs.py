...
- asset_type (railways) ✅ 
- road_type (roads) ✅ 
- hub_type (airports, iww ports, maritime ports)
...
def format_hub_type(hub_type):
    """Format hub type strings."""
    parts = hub_type.split("_")
...
if __name__ == "__main__":
    ...
    for HAZARD in HAZARDS:
        ...
        for ASSET_GEOM in HUBS:
            ...
            asset["hub_type"] = format_hub_type(ASSET_GEOM)
            hub_types.append(asset)

        df = pd.concat(hub_types, axis=0)
        ...
        for i_EPOCH, EPOCH in enumerate(EPOCHS):
            ...
            for i, asset_type in enumerate(asset_order):
                ...
                if data.empty:
                    print(f"    No data for asset type: {asset_type}, skipping.")
                    continue
            ...
            outfile = f"{outdir}/tza_hubs_polygons_{HAZARD}_{METRIC}_{range_str}_{SUBREGION if SUBREGION else 'national'}_{EPOCH}.png"
