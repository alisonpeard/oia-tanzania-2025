    ...
    for HAZARD in HAZARDS[:1]:
        for ASSET_GEOM in ASSET_GEOMS[:1]:
            ...
            if SUBREGION:
                ...
                asset_dfs = [pd.read_parquet(f).reset_index() for f in asset_files]
                asset = gpd.concat(asset_dfs, axis=0)
            ...
            for i, asset_type in enumerate(sorted(totals_epoch['asset_type'].unique())):
                ...
                cmap = create_white_to_color_cmap(CMAP[0])
                ...
                if data["scenario"].nunique() > 1:
                    ...
                    lower_err = (data["value"] - data["lower"]).values * SCALE_FACTOR
                    upper_err = (data["upper"] - data["value"]).values * SCALE_FACTOR
            ...
            fig.suptitle(
                f"hazard: {HAZARD} - asset: {ASSET_GEOM} - "
