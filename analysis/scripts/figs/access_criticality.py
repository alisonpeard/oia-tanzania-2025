...
crit_df["wdetoured"] = crit_df["wdetoured"] / 60
...
crit_disrupted = crit_diff[crit_diff[variable] > 0].copy()
...
for i, flux in enumerate(breaks):
    ...
    if i == 0:
        continue
    if i == 1:
        label = f"{'<' + f'{breaks[i]:,.0f}':<12}(q{quants[i]:.0f})"
    elif i == len(breaks) - 1:
        label = f"{'>' + f'{breaks[i-1]:,.0f}':<12}(q{quants[i-1]:.0f})"
    else:
        label = f"{f'{breaks[i-1]:,.0f} - {breaks[i]:,.0f}':<12}(q{quants[i-1]:.0f} - q{quants[i]:.0f})"
...
legend = Legend(ax, line_examples, [line.get_label() for line in line_examples],
                title=f"Annual expected\npersons {variable_labels[variable]}", loc="lower left",
                fontsize=12,
