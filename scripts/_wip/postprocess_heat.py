...
pd.options.display.max_columns = None # to review results
...
#! change input paths later
inp_dir = "/Users/alison/Downloads/pamela_results"
ref_dir = "/Users/alison/Local/github/oia-tanzania-2025/results/assets"
out_dir = "/Users/alison/Downloads/pamela_results_formatted"


if __name__ == "__main__":

    asset = "tza_roads_edges"
    
    paths = glob(f"{inp_dir}/{asset}/*/splits.geoparquet")

    for splits_path in paths:
        ...
        for sub_path in [base_path, splits_path]:
            ...
            vector_clean = prepare_roads_data(vector, vector_ref)
...
def prepare_railway_data(asset):
    def format_asset(x:str) -> str:
...
def unsplit(vector, vector_ref, hazard_cols, damage_cols, cost_cols):
    ...
    vector_grouped = vector_grouped.join(vector_ref[["geometry"]], how="outer")
    vector_grouped = vector_grouped.fillna(0.0)
    vector_grouped = gpd.GeoDataFrame(
        vector_grouped, geometry="geometry", crs="EPSG:4326"
    )
    return vector_grouped
...
if __name__ == "__main__":
    ...
    for splits_path in paths:
        ...
        # print(ref_path)
        vector_ref = gpd.read_parquet(ref_path)
        vector_splits = gpd.read_parquet(splits_path)
        ...
        vector_clean = prepare_railway_data(vector)
        vector_clean.to_parquet(out_path)
        print(f"Saved formatted profile to {out_path}")
# %%