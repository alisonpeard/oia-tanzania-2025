...
for variable, unit in zip(variable, units):
    ...
    for ax, hazard in zip(axs, hazards):
        ...
        # plot baseline
        ax.bar(x[0], baseline,
            width, label=scenario_labels['historical'], color=colors['historical'],
            linewidth=0.5, edgecolor="k")

        # plot future scenarios
        for j, ssp in enumerate(scenarios):
                means = [totals_df_hazard.loc[(hazard, e, ssp, 'mean'), hazard] for e in epochs[1:]]      
                ax.bar(x[1:] + (j-1) * width, means,
