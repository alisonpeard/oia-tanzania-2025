    ...
    for HAZARD in HAZARDS:
        ...
        for ASSET_GEOM in ASSET_GEOMS:
            ...
            if HAZARD == "landslide":
                ...
                print("WARINI: applying 0.3 factor to landslide risk metrics")
                df[metric_cols] = 0.3 * df[metric_cols]
            ...
            if HAZARD == "cyclone":
                ...
                flag_hist = totals["scenario"] == "historical"
                flag_2010 = totals["epoch"] == "2010"
                res_hist = totals[flag_2010 & flag_hist].copy()
                res_2030 = totals[flag_2010 & ~flag_hist].copy()
                res_future = totals[~flag_2010 & ~flag_hist].copy()
                res_2030["epoch"] = "2030"
                totals = pd.concat([res_hist, res_2030, res_future], axis=0, ignore_index=True)
                totals["epoch"] = totals["epoch"].replace({
                    "2090": "2080"
                })

            if ASSET_GEOM == "tza_railway_edges":
                # step !sort out historical SSPs and 2030
                flag_base = totals["epoch"].astype(int) < 2030
                flag_2030 = totals["epoch"] == "2030"
                flag_2050 = totals["epoch"] == "2050"
                flag_2080 = totals["epoch"] == "2080"

                flag_proposed = totals["asset_type"].str.contains("proposed", case=False)
                flag_planned = totals["asset_type"].str.contains("planned", case=False)
                flag_construction = totals["asset_type"].str.contains("construction", case=False)
                ...
                # totals = totals[~flag_remove].copy()
                totals.loc[flag_remove, "value"] = 0.0
